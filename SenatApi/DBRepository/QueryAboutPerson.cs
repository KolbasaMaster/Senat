...
namespace SenatApi
{
    public class QueryAboutPerson
    {
        ...
        string SqlPersonData = "SELECT ModelMembers.MemberId, COUNT(ModelMembers.IssueId) AS NumbersOfIssue, " +
        ...
        public void QueryAboutPersons()
        {
            ...
            {
                connection.Open();
                SqlCommand command = new SqlCommand(SqlPersonData, connection);
                ...
                if(reader.HasRows)
                {
                    ...
                    while (reader.Read())
                    {
                        object MemberId = reader.GetValue(0);
